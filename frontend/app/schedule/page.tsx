...
export default function SchedulePage() {
  ...
  const groupedByTherapist = useMemo(() => {
    if (view !== "day" || therapistFilter !== "all") return null;
  ...
  }, [view, therapistFilter, dayAppointments, selectedDate]);
    ...
    <>
      ...
      <main className="container space-y-6 py-8">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="text-sm text-muted-foreground">View and manage appointments across your clinic.</p>
          </div>
          <Button
            onClick={() =>
              setModal({
                ...
                initialValues: {
                  ...
                  ...(therapistFilter !== "all" ? { therapist_id: therapistFilter } : {}),
                },
              })
            }
          >
      ...
      <AlertDialog open={modal !== null} onOpenChange={(open) => !open && closeModal()}>
        <AlertDialogContent className="max-h-[90vh] max-w-lg overflow-y-auto">
          <AlertDialogHeader>
            <AlertDialogTitle>{modal?.mode === "edit" ? "Edit appointment" : "New appointment"}</AlertDialogTitle>
            <AlertDialogDescription>
              {modal?.mode === "edit"
          ...
          {modal?.mode === "edit" && modal.appointment.status !== "CANCELLED" && !confirmingCancel && (
